...
def predict(cae, c_s, rnn, r_s, image_arr, state=np.zeros((2,2,1,1024))):
    ...
    encoding = encoding.flatten().reshape((1, 1,-1))
    # get the output and state of the rnn
    rnn_output, state = r_s.run([rnn.norm_pred, rnn.state], feed_dict={rnn.x: encoding, rnn.init_state: state})
    encoding_output = rnn_output.flatten().reshape((1,2,2,4))
    # get decoded image
    decoded_output = c_s.run(cae.decoded, feed_dict={cae.encoded: encoding_output})
    return [decoded_output, state, encoding_output]
...
def main():
    ...
    state = np.zeros((2,2,1,128))
    cae, c_s, rnn, r_s = load_models(c_c, r_c)
    for i in range(sequence.shape[0]):
        frame = np.rollaxis(sequence[[i]],1,4)
        frame, state, _ = predict(cae, c_s, rnn, r_s, frame, state)
        if img is None:
