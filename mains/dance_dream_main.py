...
def dream(cae, c_s, rnn, r_s, pred=np.zeros((1,1,16)), state=np.zeros((2,2,1,256)), length=300, randomize=True):
    img = None
    ...
    while True:
        for _ in range(length):
            ...
            # pd.send2Pd(msg)
        if randomize:
            prediction = np.random.rand(1,1,16)
            init_state = np.random.rand(2,2,1,256)
