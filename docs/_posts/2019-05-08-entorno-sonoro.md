...
El entorno sonoro de Sinestes.IA va a ser el medio por el cual la inteligencia artificial podrá interactuar de manera musical. Por esta razón, debe ser diseñado con el propósito de ofrecer la mayor cantidad de variedad en un número reducido de parámetros. De esta manera, el modelo de inteligencia artificial no se verá limitado por las posibilidades de interpretación, sino por sus capacidades de aprendizaje. Además de esto, el estado del entorno sonoro debe poder ser completamente representado en todo momento por los parámetros que el programa va a manipular, lo que significa que el algoritmo tendrá control absoluto de lo que está sonando en este entorno.

Para lograr representar todo el estado del entorno sonoro, los parámetros no solo deben capturar la información de intensidad y altura de cada voz, sino que además deben capturar las características tímbricas y temporales de cada sonido. Esto lo diferencia del protocolo MIDI, ya que este no encapsula la información sonora, sino las instrucciones de interpretación que debe manejar el instrumento que uno decida. Para lograr esto, se desarrollará un instrumento como un componente aislado del entorno, el cual encapsula todos los requerimientos anteriores. Una vez implementado el instrumento, tan solo es necesario duplicar su estructura para aumentar la variedad textural.
